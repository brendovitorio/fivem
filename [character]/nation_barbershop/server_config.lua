...
function func.checkPermission(permission, src)
    ...
    if type(permission) == "table" then
        for i, perm in pairs(permission) do
            if rEVOLT.HasGroup(user_id, perm) then
                return true
            end
        end
        return false
...
function func.saveChar(char)
    ...
    rEVOLT._setUData(user_id, "nation_char", json.encode(char,{indent=false}))
    return true
end
...
function func.checkChair(barberId, chairIndex, perm)
    ...
    if (perm and not func.HasGroup(perm, source)) or not canEnter then return end
...
function func.getOverlay()
    ...
    local user_id = rEVOLT.getUserId(source)
    if user_id then
        local data = rEVOLT.getUData(user_id, "nation_char")
...
function getUserClothes(user_id)
    local data = rEVOLT.getUData(user_id, "Clothings")
    if data and data ~= "" then
        local clothes = data
        if clothes then
            return clothes
    ...
    local datatable = rEVOLT.getUserDataTable(user_id) or {}
    return datatable.customization or {}
end


function setPlayerTattoos(source, user_id)
    local data = rEVOLT.getUData(user_id,"Tatuagens")
    if data and data ~= "" then 
        tattoos = data
...
function getGender(user_id)
    local datatable = rEVOLT.getUserDataTable(user_id) or rEVOLT.getUData(user_id, "Datatable") or {}
...
--AddEventHandler("rEVOLT:playerSpawn",playerSpawn) -- caso nao use nation_creator, descomente
