    ...
    'query'(c, d) {
        ...
        return new Promise((j, k) => {
            switch (this.driver) {
                ...
                case 'hydra':
                return j(global.exports.revolt.query(c, f));
...
pp = po.getInterface('Revolt'),
pq = {
    '__proto__': null
};
pq.VProxy = po, pq.Revolt = pp;
...
if (pB.base === 'creative_v3') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (c, d) => {
...
}, pp.getUsersByPermission = c => pc('revolt_permissions').where({
    'permiss': c
}).pluck('user_id');else {
    if (pB.base === 'creative_v4') pp.getBankMoney = pp.getBank, pp.setBankMoney = async (d, e) => {
        ...
        } else pB.base === 'bahamas_network' && (pB.serverSideProp = true, pp.getUserSource = pp.Source, pp.getUserId = pp.Passport, pp.getInventoryItemAmount = pp.InventoryItemAmount, pp.HasGroup = pp.HasPermission, pp.request = pp.Request, pp.getBankMoney = async c => {
...
(pB.base === 'creative_v4' || pB.base === 'new_bahamas') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    ...
    for (const e of Object.keys(await pp.userList()).map(Number)) {
        if ((await pp.HasGroup(e, c)) || (await pp.hasGroup(e, c))) d.push(e);
    }
...
async function pE(d, e) {
    ...
    if (Array.isArray(e)) {
        if (pF.hasTable('revolt_permissions')) {
            const i = {};
            return i.user_id = d, pc('revolt_permissions').where(i).whereIn('permiss', e).exists();
        }
        
        for (let j of e) {
            if (await pp.HasGroup(d, j)) return true;
        }
        
        return false;
    } else return pp.HasGroup(d, e);
}
...
const pF = {
        ...
        return new Promise(async c => {
            ...
            if (pF.tables.includes('summerz_characters') || pF.hasTable('summerz_dummy')) {
                ...
                if (pF.hasTable('nyo_character')) {
                    const p = ['getUserSource', 'getUserId', 'getBankMoney', 'setBankMoney', 'getUsersByPermission', 'HasGroup', 'getInventoryItemAmount', 'request'];
                    p.forEach(q => {
    ...
    get 'userDataTable'() {
        return pF.firstTable('revolt_user_data', 'zuser_data', 'summerz_playerdata');
    },
    ...
    async 'getIdentitiesBy'(c = 'user_id', d = []) {
        ...
        let e;
        const f = pF.firstTable('revolt_user_infos', 'revolt_characters', 'revolt_characterdata');
                ...
                if (pF.hasTable('summerz_characters')) c = c == 'user_id' ? 'id' : c, e = await pc('summerz_characters').whereIn(c, d).select('id', 'name', 'name2', 'phone');else {
                    if (pF.hasTable('summerz_dummy')) {
                        ...
                        if (f) {
                            ...
                            if (f === 'revolt_characterdata') o[1] = 'lastname';
                            ...
                            if (pF.hasTable('zusers')) e = await pc('zusers').whereIn(c == 'user_id' ? 'id' : c, d);else {
                                if (pF.hasTable('characters')) {
                                    ...
                                    if ((await pF.hasColumn('revolt_users', 'phone')) && (await pF.firstColumn('revolt_users', 'name', 'nome', 'firstname', 'name2'))) {
                                        if (c == 'phone' && (await pF.hasColumn('revolt_users', 'dphone'))) {
                                            e = await pc('revolt_users').whereIn('phone', d).orWhereIn('dphone', d);
                                        } else {
                                            e = await pc('revolt_users').whereIn(c == 'user_id' ? 'id' : c, d);
                                        }
                                    } else {
                                        if (pF.hasTable('revolt_user_identities')) {
                                            if ((await pF.hasColumn('revolt_user_identities', 'telefone')) && c === 'phone') {
                                                c = 'telefone';
                                            }
                                            
                                            e = await pc('revolt_user_identities').whereIn(c, d);
                                        } else {
...
pL.getSettings = async f => {
    ...
    g = {};
    g.id = e;
    const h = await pc('revolt_users').where(g).first();
    if (!h) return;
...
}, on('Revolt:playerLeave', (c, d) => pL.endPhoneCall(d));
const qu = {
    '__proto__': null
};
...
async function qx(c) {
    ...
    e = pF.firstTable('revolt_user_identities', 'revolt_characters');
...
async function qy(d, e) {
    ...
    const f = await pM(d),
    g = ['revolt_user_moneys', 'revolt_user_infos', 'revolt_users_infos', 'revolt_characters', 'revolt_characterdata', 'revolt_user_identities', 'revolt_users', 'zusers', 'drip_characters', 'summerz_characters', 'characters', 'nyo_character'];
...
async function qA({
    ...
    const o = pF.firstTable('revolt_banco', 'revolt_bdl_banco', 'smartphone_bank');
    
    if (await pF.hasColumns('revolt_banco', 'title', 'amount', 'idtrans')) {
        const C = {};
...
pF.ready(async () => {
    ...
    if (await pF.hasColumn('revolt_user_moneys', 'pix')) {
        exports('getUserIdByPixKey', async e => {
            const f = {};
            f.pix = e;
            const g = await pc('revolt_user_moneys').where(f).first();
    ...
    for (let [k, l, m] of [['revolt_fines', 'text', 'price'], ['characters_fines', 'text', 'price'], ['wise_multas', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['revolt_multas', 'motivo', 'valor']]) {
        e.push({
            'table': k,
    ...
    if (pF.hasTable('smartbank_fines')) return qE = {
        ...
        async 'deleteOne'(o, p) {
            ...
            s = JSON.parse((await pp.getUData(p, 'Revolt:multas')) || '0');
            return await pp.setUData(p, 'Revolt:multas', Math.max(0, s - r.amount)), pc('smartbank_fines').destroy(o);
        }
        ...
        if (pB.base === 'new_bahamas') {
            ...
            if (pF.hasTable('drip_items')) {
                ...
                if (pF.hasTable('revolt_user_data')) {
                    e.push({
                        'table': 'revolt_user_data',
                        'applicable': true,
                        'getSum': s => pc('revolt_user_data').where({
                            'user_id': s,
                            'dkey': 'Revolt:multas'
                        }).sum('dvalue'),
                        'getAll': e[0].getAll,
    ...
    i = pF.firstTable('revolt_banco', 'revolt_bdl_banco', 'smartphone_bank');
...
qJ.kickall = c => {
    ...
    global.emit('Revolt:proxy', c, d.map(e => Number(e) || e), 'any', -1);
}, qJ.global = (c, ...d) => {
    global[c](...d);
}, qJ.revolt_all = async (c, ...d) => {
    for (const e of qI()) {
        const g = await pp.getUserId(e);
        global.emit('Revolt:proxy', c, [g].concat(d), 'any', -1);
    }
}, qJ.emit_all = (c, ...d) => {
...
global.Citizen.revoltNativeByHash(0, 4123407116, _mfr((c, d) => {
...
}, on('Revolt:playerLeave', c => delete r0.users[c]);
const r4 = {
    '__proto__': null
};
...
const r6 = {
    'table': 'revolt_user_moneys',
    'key': 'user_id'
};
const r7 = r6,
r8 = ['nyo_character', 'revolt_user_moneys', 'revolt_users', 'summerz_characters', 'revolt_characters', 'characters', 'zusers'];
pF.ready(async d => {
...
async function r9({
    ...
    return parseInt(await pc('revolt_user_moneys').where('user_id', d).first().pluck('paypal')) || 0;
}

async function ra(d, e) {
    const f = {};
    f.paypal = e, await pc('revolt_user_moneys').update(f).where('user_id', d);
}

async function rb(c, d) {
    await pc('revolt_user_moneys').update().increment('paypal', d).where('user_id', c);
}

async function rc(c, d) {
    await pc('revolt_user_moneys').update().decrement('paypal', d).where('user_id', c);
}
...
}, on('Revolt:playerLeave', c => delete ru.users[c]);
const rz = {
    '__proto__': null
};
