...
function Creative.Check()
	...
	if Passport then
		if os.time() > Active then
			...
			if rEVOLT.TakeItem(Passport,"luckywheelpass",1,true) then
				Active = os.time() + 20
				return true
			end

			if rEVOLT.PaymentBank(Passport,5000) then
				Active = os.time() + 20
				return true
			else
...
function Creative.Payment()
	...
	if Passport and not Actived[Passport] and Payments[Passport] then
		...
		elseif Payments[Passport] == 20 then
			rEVOLT.GiveBank(Passport,25000)
		end
