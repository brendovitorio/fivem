RegisterNetEvent("inventory:clientUseItem", function(itemKey, rawItem)
    local raw = tostring(itemKey or rawItem or "")
    local key = string.lower((raw:match("^[^-]+") or raw))

    if key == "radio" then
        if GetResourceState("radio") == "started" or GetResourceState("pma-voice") == "started" then
            ExecuteCommand("radio")
            return
        end

        TriggerEvent("Notify", "aviso", "O item rádio foi usado, mas nenhum recurso de rádio compatível foi encontrado.", 5)
    end
end)
...
RegisterNetEvent('inventory:useDrugs', function(drug)
	...
	if not inUseDrug then
		...
		elseif drug == "cogumelo" then
			...
			local maxHealth = math.floor((GetPedMaxHealth(ped) - 50) * 0.75)
			local maxArmor = 75

			local targetHealth = currentHealth + 25
			if targetHealth > maxHealth then
				targetHealth = maxHealth
			end

			local targetArmor = currentArmor + 25
			if targetArmor > maxArmor then
				targetArmor = maxArmor
			end
			...
			CreateThread(function()
				while inUseDrug do
					...
					if currentHealth < targetHealth then
						currentHealth = parseInt(currentHealth) + parseInt(healthIncrement)
						if currentHealth > targetHealth then
							currentHealth = targetHealth
						end
						SetEntityHealth(ped, parseInt(currentHealth))
						lastHealth = parseInt(currentHealth)
					end
...
RegisterNetEvent('repararmotor', function(vehicle)
	TriggerServerEvent("tryrepararmotor", VehToNet(vehicle))
end)

RegisterNetEvent('syncrepararmotor', function(index)
	if NetworkDoesNetworkIdExist(index) then
		local v = NetToVeh(index)
		local fuel = GetVehicleFuelLevel(v)
		if DoesEntityExist(v) then
...
RegisterNetEvent('repararlataria', function(vehicle)
	TriggerServerEvent("tryrepararlataria", VehToNet(vehicle))
end)

RegisterNetEvent('syncrepararlataria', function(index)
	if NetworkDoesNetworkIdExist(index) then
		local v = NetToVeh(index)
		local fuel = GetVehicleFuelLevel(v)
		if DoesEntityExist(v) then
			if IsEntityAVehicle(v) then
				...
				SetVehicleFixed(v)
				SetVehicleDirtLevel(v, 0.0)
				SetVehicleFuelLevel(v, fuel) -- mantém o combustível
			end
		end
	end
end)
