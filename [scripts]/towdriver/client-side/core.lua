...
-- TOWDRIVER:INVOKETOW
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("towdriver:revoltTow")
AddEventHandler("towdriver:revoltTow",function()
	local Ped = PlayerPedId()
	...
	if (GetEntityModel(Vehicle) == GetHashKey("flatbed") or GetEntityModel(Vehicle) == GetHashKey("flatbed2")) and not IsPedInAnyVehicle(Ped) then
		local vehTowed = rEVOLT.ClosestVehicle(10)
			...
			if vehDistance <= 15 then
				if inTowed then
					...
					if Vehicle ~= vehTowed then
						...
						rEVOLT.playAnim(false,{"mini@repair","fixing_a_player"},true)
