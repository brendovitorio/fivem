...
AddStateBagChangeHandler("Sirens", nil, function(Name, Key, Value)
	-- Name vem tipo "entity:123"
	local netId = tonumber((Name or ""):gsub("entity:", ""), 10)
	if not netId then return end

	if NetworkDoesNetworkIdExist(netId) then
		local Vehicle = NetToVeh(netId)
		if DoesEntityExist(Vehicle) then
			-- Value deve ser uma tabela: { lx, air }
			if type(Value) ~= "table" then return end

			local lx  = tonumber(Value[1]) or 0
			local air = tonumber(Value[2]) or 0

			TogMuteDfltSrnForVeh(Vehicle, true)
			SetLxSirenStateForVeh(Vehicle, lx)
