...
func = Tunnel.getInterface("revolt_concessionaria")
fclient = {}
Tunnel.bindInterface("revolt_concessionaria", fclient)
...
local nearestConce = {
		close = true,
		conce = vec3(-33.15, -1097.21, 27.27), 
		test_locais = {
...
RegisterNetEvent("revoltConce:setConfig")
AddEventHandler(
	"revoltConce:setConfig",
	function(cfg)
        config = cfg
    end
)
...
Citizen.CreateThread(function()
    ...
    while not config do
        TriggerServerEvent("revoltConce:getConfig")
         Citizen.Wait(1000)
    end
end)
        ...
        if mhash then
            ...
            local closestVehicleOnSpot = GetClosestVehicle(spawnCoords.x,spawnCoords.y,spawnCoords.z,3.001,0,71)
                if DoesEntityExist(closestVehicleOnSpot) then
                    ...
                    local plate = rEVOLT.getRegistrationNumber()
