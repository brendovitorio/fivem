            ...
            for Number,v in pairs(Textform) do
                ...
                if Distance <= 15 and GetGameTimer() <= v["Seconds"] then
                    ...
                    if not Showform[Number] then
                        HudSend({ Action = "Textform", Mode = "Create", Number = Number, x = X, y = Y })
                        Showform[Number] = true
                    end

                    HudSend({ Action = "Textform", Mode = "Update", Text = v["Text"], Number = Number, x = X, y = Y })
                else
                    if Showform[Number] then
                        HudSend({ Action = "Textform", Mode = "Remove", Number = Number })
                        Showform[Number] = nil
                    end
                end
            end
